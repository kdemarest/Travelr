...
export class PanelPlan extends LitElement {
  ...
  private handleDateClick(line: PlanLine) {
    ...
    this.dispatchEvent(
      new CustomEvent("plan-date-selected", {
        detail: { line },
        bubbles: true,
        composed: true
      })
  ...
  private getDateRangeLabel(): string | null {
    const datedLines = this.lines
      ...
      .map((line) => {
        const parsed = new Date(line.dateKey);
        const timestamp = parsed.getTime();
        return Number.isNaN(timestamp)
          ? null
          : { line, timestamp, year: parsed.getFullYear() };
      })
      .filter(
    ...
    if (first.line.dateKey === last.line.dateKey) {
      return format(first);
    }
  ...
  protected updated(changedProps: PropertyValues<this>) {
    ...
    if ((changedProps.has("selectedKey") || changedProps.has("lines")) && this.selectedKey) {
      this.ensureSelectedDateVisible();
    }
  }

  private ensureSelectedDateVisible() {
    const container = this.renderRoot.querySelector<HTMLElement>(".agenda");
    if (!container || !this.selectedKey) {
      return;
    }
    ...
    // Keep selected plan row within the scroll frame so first visit highlights correctly.
  ...
  private buildDateLineClasses(line: Extract<PlanLine, { kind: "dated" }>): string {
    ...
    if (line.dateKey === this.selectedKey) {
      classes.push("selected");
    }
    if (line.dateKey === this.hoveredKey) {
      classes.push("hovered");
    }
    if (line.dateKey === this.dropTargetKey) {
      classes.push("drop-target");
    }
    if (line.dateKey === this.draggingKey) {
      classes.push("dragging", "dragging-source");
    }
    if (this.incomingActivityDrag?.dateKey === line.dateKey) {
  ...
  private renderDatedLine(line: Extract<PlanLine, { kind: "dated" }>) {
    ...
    const hoverDisplay =
      this.hoveredKey === line.dateKey && line.activities.length > 0
        ? this.formatActivityCount(line.activities.length)
        : notation;
    return html`<div
      class=${this.buildDateLineClasses(line)}
      data-key=${line.dateKey}
      @click=${() => this.handleDateClick(line)}
      @mouseenter=${() => this.setHoveredKey(line.dateKey)}
  ...
  private handleDragHandlePointerDown(event: PointerEvent, line: Extract<PlanLine, { kind: "dated" }>) {
    ...
    this.draggingKey = line.dateKey;
    this.dropTargetKey = line.dateKey;
    this.updateGhostDateForKey(line.dateKey);
    console.log("[panel-plan] drag start", line.dateKey);
  ...
  private updateGhostDateForKey(key: string | null) {
    ...
    const targetLine = this.lines.find(
      (line): line is Extract<PlanLine, { kind: "dated" }> => line.kind === "dated" && line.dateKey === key
    );
    if (!targetLine) {
      return;
    }
